...
module.exports = {
  ...
  async validateInput(req, objectName, idIsRequired) {
    ...
    for (let i = 0; i < attributes.length; i++) {
      ...
      if (attribute === 'id' && !idIsRequired) {
        ...
        isRequired &&
        req.body[attribute]['id'] === undefined
      ) {
        return {
          validationMessage: `Parâmetro ${attribute}.id é obrigatório.`
        };
      }

      if (isRequired && req.body[attribute] === undefined) {
        return {
          validationMessage: `Parâmetro ${attribute} é obrigatório.`
        };
      ...
      if (isRequired && type === 'date-time') {
        dateFormats = [
          'YYYY-MM-DDTHH:mm:ss.sss+HH:mm' /**moment.ISO_8601 */
        ];
        if (!moment(inputReq[attribute], dateFormats, true).isValid())
          return {
            validationMessage: `${attribute} = ${
              inputReq[attribute]
            } não está no formato YYYY-MM-DDTHH:mm:ss.sss+HH:mm.`
          };
        ...
        if (!moment(inputReq[attribute], dateFormats, true).isValid())
          return {
            validationMessage: `${attribute} = ${
              inputReq[attribute]
            } não está no formato YYYY-MM-DD`
          };
        ...
        if (
          ...
          return {
            validationMessage: `${attribute}.id = ${
              inputReq[attribute]
            } não é um número de id válido.`
          };
      }
    }
    return inputReq;
  ...
  relationIdMongo(req, res, statusCode, objName, err, obj, id) {
    if (err) {
      this.retErr(req, res, statusCode, {
        msg: `Banco de dados nao conseguiu consultar ${objName}: ${
          err.message
        }`
      });
    } else if (!obj) {
      this.retErr(req, res, statusCode, {
        msg: `Não foi encontrado nenhum ${objName} com o id = ${id}.`
      });
    } else {
      return obj._id;
    }
  ...
  retErr(req, res, statusCode, msg) {
    ...
    let desc =
      protocols.paths[`${path}`].methods[`${method}`].responses[
        `${statusCode}`
      ].description;
    console.log(desc);
    console.log(msg);
    return res
      .status(statusCode)
      .json({ description: desc, message: msg });
  },
  ...
  retOk(req, res, statusCode, obj) {
    ...
    if (path === '/user' && method === 'POST' && statusCode === 201) {
      obj.password =
        'Sua senha está segura em nossa base de dados! :D';
