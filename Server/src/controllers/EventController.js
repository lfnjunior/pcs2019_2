    ...
    try {
      ...
      if (start_date && end_date) {
        ...
        if (!moment(start_date, dateFormat, true).isValid())
          return Utils.retErr(res, `start_date = ${start_date} não está no formato YYYY-MM-DDTHH:mm:ss.SSSZ`)
        if (!moment(end_date, dateFormat, true).isValid())
          return Utils.retErr(res, `end_date = ${end_date} não está no formato YYYY-MM-DDTHH:mm:ss.SSSZ`)
